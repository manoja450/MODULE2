...
struct pulse {
    ...
    double veto_energy[10]; // Energy for each veto panel (12-21)
};
...
int main(int argc, char *argv[]) {
    ...
    int num_michels = 0;
    int num_events = 0;
    int num_single_thin_vp_muons[4] = {0}; // Counters for each thin panel (18-21)
    int num_triple_coincidence_muons[4] = {0}; // Counters for triple coincidence (top panels + thin panel)
    int num_no_thin_vp_muons = 0; // Counter for muons without thin panel hits
    int num_no_thin_vp_michels = 0; // Counter for Michels from muons without thin panel hits
    ...
    // Additional histograms for single thin panel muons
    TH1D* h_muon_energy_vp18 = new TH1D("muon_energy_vp18", "Muon Energy (Only Veto Panel 18);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_vp19 = new TH1D("muon_energy_vp19", "Muon Energy (Only Veto Panel 19);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_vp20 = new TH1D("muon_energy_vp20", "Muon Energy (Only Veto Panel 20);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_vp21 = new TH1D("muon_energy_vp21", "Muon Energy (Only Veto Panel 21);Energy (p.e.);Counts", 550, -500, 5000);

    // Histograms for triple coincidence (top panels + thin panel)
    TH1D* h_muon_energy_top_vp18 = new TH1D("muon_energy_top_vp18", "Muon Energy (Top Panels + Veto Panel 18);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_top_vp19 = new TH1D("muon_energy_top_vp19", "Muon Energy (Top Panels + Veto Panel 19);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_top_vp20 = new TH1D("muon_energy_top_vp20", "Muon Energy (Top Panels + Veto Panel 20);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_muon_energy_top_vp21 = new TH1D("muon_energy_top_vp21", "Muon Energy (Top Panels + Veto Panel 21);Energy (p.e.);Counts", 550, -500, 5000);

    // New histograms for SiPM energy spectra in thin side veto panels (18-21)
    // Single coincidence (only one thin panel hit)
    TH1D* h_sipm_single_vp18 = new TH1D("sipm_single_vp18", "SiPM Energy (Single VP18);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_single_vp19 = new TH1D("sipm_single_vp19", "SiPM Energy (Single VP19);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_single_vp20 = new TH1D("sipm_single_vp20", "SiPM Energy (Single VP20);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_single_vp21 = new TH1D("sipm_single_vp21", "SiPM Energy (Single VP21);Energy (ADC);Counts", 200, 0, 8000);

    // Triple coincidence (top panels + one thin panel)
    TH1D* h_sipm_triple_vp18 = new TH1D("sipm_triple_vp18", "SiPM Energy (Triple VP12+13+18);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_triple_vp19 = new TH1D("sipm_triple_vp19", "SiPM Energy (Triple VP12+13+19);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_triple_vp20 = new TH1D("sipm_triple_vp20", "SiPM Energy (Triple VP12+13+20);Energy (ADC);Counts", 200, 0, 8000);
    TH1D* h_sipm_triple_vp21 = new TH1D("sipm_triple_vp21", "SiPM Energy (Triple VP12+13+21);Energy (ADC);Counts", 200, 0, 8000);

    // New histograms for analysis without thin side veto panels
    TH1D* h_muon_energy_no_thin_vp = new TH1D("muon_energy_no_thin_vp", "Muon Energy (No Thin Veto Panels);Energy (p.e.);Counts", 550, -500, 5000);
    TH1D* h_michel_energy_no_thin_vp = new TH1D("michel_energy_no_thin_vp", "Michel Electron Energy (No Thin Veto Panels);Energy (p.e.);Counts", 100, 0, 800);
    TH1D* h_dt_michel_no_thin_vp = new TH1D("DeltaT_no_thin_vp", "Muon-Michel Time Difference (No Thin Veto Panels);Time to Previous event(Muon)(#mus);Counts/0.08 #mus", 200, 0, MICHEL_DT_MAX);
    double last_muon_time_no_thin_vp = 0.0;
    std::set<double> michel_muon_times_no_thin_vp;
    ...
    for (const auto& inputFileName : inputFiles) {
        ...
        for (int iEnt = 0; iEnt < numEntries; iEnt++) {
            ...
            for (int iChan = 0; iChan < 23; iChan++) {
                ...
                if (iChan >= 12 && iChan <= 21) {
                    p.veto_energy[iChan-12] = allPulseEnergy;
            ...
            // Additional logic for single thin panel muons
            bool single_thin_vp_hit = false;
            int hit_thin_panel = -1;
            int thin_panel_hit_count = 0;
            
            // Check thin side panels only (18-21, indices 6-9 in veto_hit array)
            for (int i = 6; i < 10; i++) {
                if (p.veto_hit[i]) {
                    thin_panel_hit_count++;
                    hit_thin_panel = i;
                }
            }
            
            // Check that no other panels were hit
            bool other_panels_hit = false;
            for (int i = 0; i < 6; i++) { // Check top and wide side panels (12-17)
                if (p.veto_hit[i]) {
                    other_panels_hit = true;
                    break;
                }
            }
            
            single_thin_vp_hit = (thin_panel_hit_count == 1) && !other_panels_hit;
            
            if (single_thin_vp_hit && ((p.energy > MUON_ENERGY_THRESHOLD) ||
                (pulse_at_end && p.energy > MUON_ENERGY_THRESHOLD / 2))) {
                // Fill the appropriate histogram based on which panel was hit
                switch(hit_thin_panel) {
                    case 6: // Panel 18
                        h_muon_energy_vp18->Fill(p.energy);
                        h_sipm_single_vp18->Fill(p.veto_energy[6]); // Fill SiPM energy
                        num_single_thin_vp_muons[0]++;
                        break;
                    case 7: // Panel 19
                        h_muon_energy_vp19->Fill(p.energy);
                        h_sipm_single_vp19->Fill(p.veto_energy[7]); // Fill SiPM energy
                        num_single_thin_vp_muons[1]++;
                        break;
                    case 8: // Panel 20
                        h_muon_energy_vp20->Fill(p.energy);
                        h_sipm_single_vp20->Fill(p.veto_energy[8]); // Fill SiPM energy
                        num_single_thin_vp_muons[2]++;
                        break;
                    case 9: // Panel 21
                        h_muon_energy_vp21->Fill(p.energy);
                        h_sipm_single_vp21->Fill(p.veto_energy[9]); // Fill SiPM energy
                        num_single_thin_vp_muons[3]++;
                        break;
                }
            }

            // Additional logic for triple coincidence (both top panels + one thin panel)
            bool triple_coincidence = false;
            int hit_thin_panel_triple = -1;
            
            // Check if both top panels were hit
            bool both_top_hit = p.veto_hit[0] && p.veto_hit[1]; // Panels 12 and 13
            
            // Check if exactly one thin panel was hit
            int thin_panel_hit_count_triple = 0;
            for (int i = 6; i < 10; i++) {
                if (p.veto_hit[i]) {
                    thin_panel_hit_count_triple++;
                    hit_thin_panel_triple = i;
                }
            }
            
            // Check that no wide side panels were hit
            bool wide_side_hit = false;
            for (int i = 2; i < 6; i++) { // Panels 14-17
                if (p.veto_hit[i]) {
                    wide_side_hit = true;
                    break;
                }
            ...
            triple_coincidence = both_top_hit && (thin_panel_hit_count_triple == 1) && !wide_side_hit;
            
            if (triple_coincidence && ((p.energy > MUON_ENERGY_THRESHOLD) ||
                (pulse_at_end && p.energy > MUON_ENERGY_THRESHOLD / 2))) {
                // Fill the appropriate histogram based on which thin panel was hit
                switch(hit_thin_panel_triple) {
                    case 6: // Panel 18
                        h_muon_energy_top_vp18->Fill(p.energy);
                        h_sipm_triple_vp18->Fill(p.veto_energy[6]); // Fill SiPM energy
                        num_triple_coincidence_muons[0]++;
                        break;
                    case 7: // Panel 19
                        h_muon_energy_top_vp19->Fill(p.energy);
                        h_sipm_triple_vp19->Fill(p.veto_energy[7]); // Fill SiPM energy
                        num_triple_coincidence_muons[1]++;
                        break;
                    case 8: // Panel 20
                        h_muon_energy_top_vp20->Fill(p.energy);
                        h_sipm_triple_vp20->Fill(p.veto_energy[8]); // Fill SiPM energy
                        num_triple_coincidence_muons[2]++;
                        break;
                    case 9: // Panel 21
                        h_muon_energy_top_vp21->Fill(p.energy);
                        h_sipm_triple_vp21->Fill(p.veto_energy[9]); // Fill SiPM energy
                        num_triple_coincidence_muons[3]++;
                        break;
                }
            }

            // New analysis: Muons without thin side veto panels (channels 18-21)
            bool no_thin_vp_hit = true;
            for (int i = 6; i < 10; i++) { // Thin side panels (18-21)
                if (p.veto_hit[i]) {
                    no_thin_vp_hit = false;
                    break;
                }
            }

            // Check if any other veto panels were hit (12-17)
            bool other_vp_hit = false;
            for (int i = 0; i < 6; i++) {
                if (p.veto_hit[i]) {
                    other_vp_hit = true;
                    break;
            ...
            if (no_thin_vp_hit && other_vp_hit && p.energy > MUON_ENERGY_THRESHOLD) {
                h_muon_energy_no_thin_vp->Fill(p.energy);
                last_muon_time_no_thin_vp = p.start;
                num_no_thin_vp_muons++;
            }
            ...
            // New analysis: Michel electrons from muons without thin side veto panels
            double dt_no_thin_vp = p.start - last_muon_time_no_thin_vp;
            bool is_michel_no_thin_vp = no_thin_vp_hit && 
                                      p.energy >= MICHEL_ENERGY_MIN &&
                                      p.energy <= MICHEL_ENERGY_MAX &&
                                      dt_no_thin_vp >= MICHEL_DT_MIN &&
                                      dt_no_thin_vp <= MICHEL_DT_MAX &&
                                      p.number >= 8 &&
                                      veto_low &&
                                      p.trigger != 1 &&
                                      p.trigger != 4 &&
            ...
            if (is_michel_no_thin_vp) {
                h_michel_energy_no_thin_vp->Fill(p.energy);
                h_dt_michel_no_thin_vp->Fill(dt_no_thin_vp);
                michel_muon_times_no_thin_vp.insert(last_muon_time_no_thin_vp);
                num_no_thin_vp_michels++;
            }
        ...
        cout << "Muons with single thin veto panel hits:\n";
        cout << "  Panel 18: " << num_single_thin_vp_muons[0] << "\n";
        cout << "  Panel 19: " << num_single_thin_vp_muons[1] << "\n";
        cout << "  Panel 20: " << num_single_thin_vp_muons[2] << "\n";
        cout << "  Panel 21: " << num_single_thin_vp_muons[3] << "\n";
        cout << "Muons with triple coincidence (top panels + thin panel):\n";
        cout << "  Top panels + Panel 18: " << num_triple_coincidence_muons[0] << "\n";
        cout << "  Top panels + Panel 19: " << num_triple_coincidence_muons[1] << "\n";
        cout << "  Top panels + Panel 20: " << num_triple_coincidence_muons[2] << "\n";
        cout << "  Top panels + Panel 21: " << num_triple_coincidence_muons[3] << "\n";
        cout << "Muons without thin side veto panels: " << num_no_thin_vp_muons << "\n";
        cout << "Michel electrons from muons without thin side veto panels: " << num_no_thin_vp_michels << "\n";
        cout << "------------------------\n";
        ...
        num_michels = 0;
        for (int i = 0; i < 4; i++) {
            num_single_thin_vp_muons[i] = 0;
            num_triple_coincidence_muons[i] = 0;
        }
        num_no_thin_vp_muons = 0;
        num_no_thin_vp_michels = 0;
    }
    ...
    // Plot single thin panel muon energy spectra
    // Panel 18
    c->Clear();
    h_muon_energy_vp18->SetLineColor(kRed);
    h_muon_energy_vp18->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_VP18.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 19
    c->Clear();
    h_muon_energy_vp19->SetLineColor(kBlue);
    h_muon_energy_vp19->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_VP19.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 20
    c->Clear();
    h_muon_energy_vp20->SetLineColor(kGreen);
    h_muon_energy_vp20->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_VP20.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 21
    c->Clear();
    h_muon_energy_vp21->SetLineColor(kMagenta);
    h_muon_energy_vp21->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_VP21.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot triple coincidence muon energy spectra
    // Top panels + Panel 18
    c->Clear();
    h_muon_energy_top_vp18->SetLineColor(kRed);
    h_muon_energy_top_vp18->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_Top_VP18.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Top panels + Panel 19
    c->Clear();
    h_muon_energy_top_vp19->SetLineColor(kBlue);
    h_muon_energy_top_vp19->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_Top_VP19.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Top panels + Panel 20
    c->Clear();
    h_muon_energy_top_vp20->SetLineColor(kGreen);
    h_muon_energy_top_vp20->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_Top_VP20.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Top panels + Panel 21
    c->Clear();
    h_muon_energy_top_vp21->SetLineColor(kMagenta);
    h_muon_energy_top_vp21->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_Top_VP21.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot SiPM energy spectra for single coincidence
    // Panel 18
    c->Clear();
    h_sipm_single_vp18->SetLineColor(kRed);
    h_sipm_single_vp18->SetLineWidth(2);
    h_sipm_single_vp18->SetTitle("SiPM Energy (Single VP18)");
    h_sipm_single_vp18->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Single_VP18.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 19
    c->Clear();
    h_sipm_single_vp19->SetLineColor(kBlue);
    h_sipm_single_vp19->SetLineWidth(2);
    h_sipm_single_vp19->SetTitle("SiPM Energy (Single VP19)");
    h_sipm_single_vp19->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Single_VP19.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 20
    c->Clear();
    h_sipm_single_vp20->SetLineColor(kGreen);
    h_sipm_single_vp20->SetLineWidth(2);
    h_sipm_single_vp20->SetTitle("SiPM Energy (Single VP20)");
    h_sipm_single_vp20->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Single_VP20.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 21
    c->Clear();
    h_sipm_single_vp21->SetLineColor(kMagenta);
    h_sipm_single_vp21->SetLineWidth(2);
    h_sipm_single_vp21->SetTitle("SiPM Energy (Single VP21)");
    h_sipm_single_vp21->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Single_VP21.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot SiPM energy spectra for triple coincidence
    // Panel 18
    c->Clear();
    h_sipm_triple_vp18->SetLineColor(kRed);
    h_sipm_triple_vp18->SetLineWidth(2);
    h_sipm_triple_vp18->SetTitle("SiPM Energy (Triple VP12+13+18)");
    h_sipm_triple_vp18->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Triple_VP18.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 19
    c->Clear();
    h_sipm_triple_vp19->SetLineColor(kBlue);
    h_sipm_triple_vp19->SetLineWidth(2);
    h_sipm_triple_vp19->SetTitle("SiPM Energy (Triple VP12+13+19)");
    h_sipm_triple_vp19->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Triple_VP19.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 20
    c->Clear();
    h_sipm_triple_vp20->SetLineColor(kGreen);
    h_sipm_triple_vp20->SetLineWidth(2);
    h_sipm_triple_vp20->SetTitle("SiPM Energy (Triple VP12+13+20)");
    h_sipm_triple_vp20->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Triple_VP20.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Panel 21
    c->Clear();
    h_sipm_triple_vp21->SetLineColor(kMagenta);
    h_sipm_triple_vp21->SetLineWidth(2);
    h_sipm_triple_vp21->SetTitle("SiPM Energy (Triple VP12+13+21)");
    h_sipm_triple_vp21->Draw("hist");
    c->Update();
    plotName = OUTPUT_DIR + "/SiPM_Triple_VP21.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot new analysis: Muons without thin side veto panels
    c->Clear();
    h_muon_energy_no_thin_vp->SetLineColor(kBlue);
    h_muon_energy_no_thin_vp->SetLineWidth(2);
    h_muon_energy_no_thin_vp->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Muon_Energy_No_Thin_VP.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot new analysis: Michel electrons from muons without thin side veto panels
    c->Clear();
    h_michel_energy_no_thin_vp->SetLineColor(kRed);
    h_michel_energy_no_thin_vp->SetLineWidth(2);
    h_michel_energy_no_thin_vp->Draw();
    c->Update();
    plotName = OUTPUT_DIR + "/Michel_Energy_No_Thin_VP.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;

    // Plot new analysis: Michel dt from muons without thin side veto panels
    c->Clear();
    h_dt_michel_no_thin_vp->SetLineWidth(2);
    h_dt_michel_no_thin_vp->SetLineColor(kBlack);
    h_dt_michel_no_thin_vp->GetXaxis()->SetTitle("Time to previous event (Muon) [#mus]");
    h_dt_michel_no_thin_vp->Draw("HIST");

    std::unique_ptr<TF1> expFit_no_thin_vp;
    if (h_dt_michel_no_thin_vp->GetEntries() > 5) {
        // Initial parameter estimates
        double integral = h_dt_michel_no_thin_vp->Integral(h_dt_michel_no_thin_vp->FindBin(FIT_MIN), h_dt_michel_no_thin_vp->FindBin(FIT_MAX));
        double bin_width = h_dt_michel_no_thin_vp->GetBinWidth(1);
        double N0_init = integral * bin_width / (FIT_MAX - FIT_MIN);
        double C_init = 0;
        
        // Estimate constant background from last bins (12-16 µs)
        int bin_12 = h_dt_michel_no_thin_vp->FindBin(12.0);
        int bin_16 = h_dt_michel_no_thin_vp->FindBin(16.0);
        double min_content = 1e9;
        for (int i = bin_12; i <= bin_16; i++) {
            double content = h_dt_michel_no_thin_vp->GetBinContent(i);
            if (content > 0 && content < min_content) min_content = content;
        }
        if (min_content < 1e9) C_init = min_content;
        else C_init = 0.1;
        ...
        // Create and configure fit function
        expFit_no_thin_vp.reset(new TF1("expFit_no_thin_vp", ExpFit, FIT_MIN, FIT_MAX, 3));
        expFit_no_thin_vp->SetParameters(N0_init, 2.2, C_init);
        expFit_no_thin_vp->SetParLimits(0, 0, N0_init * 100);
        expFit_no_thin_vp->SetParLimits(1, 0.1, 20.0);
        expFit_no_thin_vp->SetParLimits(2, -C_init * 10, C_init * 10);
        expFit_no_thin_vp->SetParNames("N_{0}", "#tau", "C");
        expFit_no_thin_vp->SetLineColor(kRed);
        expFit_no_thin_vp->SetLineWidth(3);

        // Perform fit and draw on top
        int fitStatus = h_dt_michel_no_thin_vp->Fit(expFit_no_thin_vp.get(), "RE+", "SAME", FIT_MIN, FIT_MAX);
        
        // Update stats box
        gPad->Update();
        TPaveStats *stats = (TPaveStats*)h_dt_michel_no_thin_vp->FindObject("stats");
        if (stats) {
            stats->SetX1NDC(0.6);
            stats->SetX2NDC(0.9);
            stats->SetY1NDC(0.6);
            ...
            stats->SetTextColor(kRed);
            stats->Clear();
            stats->AddText("DeltaT (No Thin VP)");
            stats->AddText(Form("#tau = %.4f #pm %.4f #mus", expFit_no_thin_vp->GetParameter(1), expFit_no_thin_vp->GetParError(1)));
            stats->AddText(Form("#chi^{2}/NDF = %.4f", expFit_no_thin_vp->GetChisquare() / expFit_no_thin_vp->GetNDF()));
            stats->AddText(Form("N_{0} = %.1f #pm %.1f", expFit_no_thin_vp->GetParameter(0), expFit_no_thin_vp->GetParError(0)));
            stats->AddText(Form("C = %.1f #pm %.1f", expFit_no_thin_vp->GetParameter(2), expFit_no_thin_vp->GetParError(2)));
            stats->Draw();
        }
        
        // Print fit results
        double N0 = expFit_no_thin_vp->GetParameter(0);
        double N0_err = expFit_no_thin_vp->GetParError(0);
        double tau = expFit_no_thin_vp->GetParameter(1);
        double tau_err = expFit_no_thin_vp->GetParError(1);
        double C = expFit_no_thin_vp->GetParameter(2);
        double C_err = expFit_no_thin_vp->GetParError(2);
        double chi2 = expFit_no_thin_vp->GetChisquare();
        int ndf = expFit_no_thin_vp->GetNDF();
        double chi2_ndf = ndf > 0 ? chi2 / ndf : 0;

        cout << "Exponential Fit Results (Michel dt without thin veto panels, " << FIT_MIN << "-" << FIT_MAX << " µs):\n";
        cout << "Fit Status: " << fitStatus << " (0 = success)\n";
        cout << Form("τ = %.4f ± %.4f µs", tau, tau_err) << endl;
        cout << Form("N₀ = %.1f ± %.1f", N0, N0_err) << endl;
        cout << Form("C = %.1f ± %.1f", C, C_err) << endl;
        ...
        cout << "Warning: h_dt_michel_no_thin_vp has insufficient entries (" << h_dt_michel_no_thin_vp->GetEntries() 
             << "), skipping exponential fit" << endl;
    }
    ...
    c->Modified();
    c->RedrawAxis();
    plotName = OUTPUT_DIR + "/Michel_dt_No_Thin_VP.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;
    ...
    delete h_muon_energy_vp18;
    delete h_muon_energy_vp19;
    delete h_muon_energy_vp20;
    delete h_muon_energy_vp21;
    delete h_muon_energy_top_vp18;
    delete h_muon_energy_top_vp19;
    delete h_muon_energy_top_vp20;
    delete h_muon_energy_top_vp21;
    delete h_sipm_single_vp18;
    delete h_sipm_single_vp19;
    delete h_sipm_single_vp20;
    delete h_sipm_single_vp21;
    delete h_sipm_triple_vp18;
    delete h_sipm_triple_vp19;
    delete h_sipm_triple_vp20;
    delete h_sipm_triple_vp21;
    delete h_muon_energy_no_thin_vp;
    delete h_michel_energy_no_thin_vp;
    delete h_dt_michel_no_thin_vp;
    for (int i = 0; i < 10; i++) {
